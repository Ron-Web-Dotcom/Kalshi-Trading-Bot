...
async def is_event_live_now(title: str) -> bool:
    """
    Return True if the real-world event behind a prediction market is
    happening RIGHT NOW — e.g. France playing a World Cup match, an NBA
    Finals game in progress, election night vote counting underway.

    Strategy:
      1. Extract named entities (teams, people, countries) from the title.
      2. Query SofaScore live events across all major sports in parallel.
      3. Match entity names against live home/away team names.
      4. For politics/elections: flag if today is within 1 day of a known
         election keyword AND "elect" / "midterm" / "vote" is in the title.
    """
    import asyncio
    ...
    raw_entities = re.findall(r"\b[A-Z][a-zA-Z]+(?:\s+[A-Z][a-zA-Z]+)*\b", title)
    # Also grab short country/city names that may be lowercase in the market title
    extra = re.findall(r"\b[a-z]{3,}\b", title_lower)
    entities = [e.lower() for e in raw_entities[:6]] + extra[:6]
    entities = list(dict.fromkeys(entities))   # deduplicate, preserve order

    # ── Politics heuristic: election day detection ────────────────────────────
    _ELECTION_KEYWORDS = ["elect", "midterm", "ballot", "vote", "primary", "runoff",
                          "inaugur", "senator", "congress", "governor"]
    if any(k in title_lower for k in _ELECTION_KEYWORDS):
        # On election day itself (US: first Tuesday of November) flag as live
        now = datetime.now(timezone.utc)
        # Election day proxy: November, Tuesday
        if now.month == 11 and now.weekday() == 1 and 2 <= now.day <= 8:
            return True
        # Also flag on primary days (March–June, Tuesday)
        if now.month in (3, 4, 5, 6) and now.weekday() == 1:
            return True

    # ── Sports: check SofaScore live events across all sports ────────────────
    sport_slugs = list(dict.fromkeys(_SOFA_SPORTS.values()))   # unique slugs

    async def _check_sport(slug: str) -> bool:
        events = await fetch_sofa_live(slug)
        for ev in events:
            home = (ev.get("home") or "").lower()
            away = (ev.get("away") or "").lower()
            tourney = (ev.get("tournament") or "").lower()
            combined = f"{home} {away} {tourney}"
            for ent in entities:
                if len(ent) >= 4 and ent in combined:
                    logger.info(
                        "is_event_live_now: '%s' matched live event '%s vs %s' (%s)",
                        ent, ev.get("home"), ev.get("away"), ev.get("tournament"),
                    )
                    return True
        return False
    ...
    try:
        results = await asyncio.gather(
            *[_check_sport(slug) for slug in sport_slugs],
            return_exceptions=True,
        )
