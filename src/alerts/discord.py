...
class DiscordAlerter:
    ...
    async def bot_alert(self, picks: List[Dict], mode: str = "PAPER") -> None:
        ...
        def _days_out(p) -> float:
            h = _hrs_left(p)
            return h / 24 if h is not None else 999

        watching = [p for p in watching_all if 0 < _days_out(p) <= 7]
        ...
        if watching:
            ...
            tomorrow_picks = [p for p in watching if _is_tomorrow_et(p)]
    ...
    async def hourly_heartbeat(
        ...
        now_et_hb       = _hb_now_et()
        hhmm            = format_et(now_et_hb, "%I:%M %p") + f" {et_label()}"
        color           = 0x5865F2
        _hb_today_et    = now_et_hb.date()
        _hb_tomorrow_et = _hb_today_et.__class__.fromordinal(_hb_today_et.toordinal() + 1)
        _hb_tz_et       = now_et_hb.tzinfo  # ZoneInfo("America/New_York")
        ...
        def _timing(ct: str) -> str:
            ...
            try:
                ...
                hours = (close_dt - now_et_hb).total_seconds() / 3600
                if hours < 0:
                    return "⏰ Resolving now"
                if hours <= 1:
                    mins = int(hours * 60)
    ...
    async def daytime_summary(
        ...
        from src.utils.eastern_time import now_et as _now_et_ds
        ...
        def _is_today_ds(b) -> bool:
            ...
            if cd_et is None:
                return False
            return cd_et.date() == _today_ds and (cd_et - _now_ds).total_seconds() > 0
        ...
        def _hrs_to_close(b):
            ...
            if cd_et is None:
                return 999
            return (cd_et - _now_ds).total_seconds() / 3600
        ...
        if today_buys or watch_buys:
            ...
            for b in watch_buys:
                ...
                rlines.append(
                    f"👀 {plat} **{label}**\n"
                    f"   WATCHING — {conf:.0f}% conf — bid on event day ({day_lbl} {time_lbl})"
                )
            fields.append({
